...
//		foreach ($this->googleMapCfg['mainMapIds'] as $mapId)
	...
	{
		if( $this->getLayoutVersion() == BOOTSTRAP_LAYOUT )
			return '<li class="' . ( $active ? "active" : "" ) . '"><a href="#" pageNum="'.$pageNum.'" >'.$linkText.'</a></li>';
		else
		{
			if( $active )
				return "<b>" . $pageNum . "</b>";
...
*/			
	...
	public function displayMenu($menuId, $menuType) 
	{
		
		if( $menuId != WELCOME_MENU )
			$menuName = $this->pSet->getMenuName($this->xt->template_file, $menuId, $menuType);
		else
			$menuName = $menuId;
		$menuStyle = $this->pSet->getMenuStyle($this->xt->template_file, $menuId, $menuType);
		
		if( $this->isAdminTable() )
			$menuName = $menuId = "adminarea";
		
			
		$xt = new Xtempl();
		$xt->assign("menuName", $menuName); 
		$xt->assign("menustyle", $menuStyle ? "second" : "main" );
		$quickjump = ( $menuType == "quickjump" );
		$horizontal = ( $menuType == "horizontal" );
			
		if( !$this->isAdminTable() ){
			if(!$quickjump)
			{
				if( $this->pSet->isMenuTreelike( $menuId ) )
				{
			...
			if($this->pageType == PAGE_MENU && IsAdmin() && !isMobile())
		...
		$menuRoot = $this->getMenuRoot( $menuId );
		
		MenuItem::setMenuSession();
		if($this->getLayoutVersion() == 3)
		{
			$peers = $this->prepareBreadcrumbsMenu( $menuRoot, $xt );
		...
		if($quickjump)
			$xt->load_template($menuName."_"."mainmenu_quickjump.htm");
		else if($horizontal)
			$xt->load_template($menuName."_"."mainmenu_horiz.htm");
		else if(isMobile() && $this->getLayoutVersion() != 1)
			$xt->load_template($menuName."_"."mainmenu_m.htm");
		else
			$xt->load_template($menuName."_"."mainmenu.htm");
	...
	protected function getMasterDetailMenuItems( &$menuRoot,  &$currentMenuItem )
	{
		...
		while( isset( $_SESSION[ $sessionPrefix."_mastertable" ] ) )
		{
			...
			if( count( $masterTableData ) )
			{
				...
				else
				{
					...
					$items[] = array("isMenuItem" => false, "menuItem" => array( "title" => $tName, "href" => $href ), "keyTitle" => implode(", ", $keys));
				}	
		...
		if( count( $items ) )
		{
			...
			{
				$href = GetTableLink( GetTableURL( $tName ), $pType );
				$items[] = array("isMenuItem" => false, "menuItem" => array( "title" => $tName, "href" => $href ) );
			}
		}
	 ...
	 *
	 */
	protected function prepareBreadcrumbsMenu( $menuRoot, $xt )
	{
	...
	public function getMenuRoot( $menuId )
	{
		if( !isset( $this->menuRoots[ $menuId ] ) )
		{
			...
			$menuMap = array();
			$this->menuRoots[ $menuId ] = new MenuItem($rootInfoArr, $menuNodes, $nullParent, $menuMap, $this, $menuId );
			
		}
		return $this->menuRoots[ $menuId ];
	}
	public function fillControlFlags( $field )
	{
		if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT || !$this->pSet->isRequired( $field ) )
			return;
