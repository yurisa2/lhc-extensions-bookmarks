...
class Chart
{
	...
	function write()
	{
		...
		echo "<charts>"."\n";
		*/
		$chart["series"] = $this->write_data();
		//$chart["dataPlotSettings"] = $this->write_dps();
		//$settings = $this->write_chart_settings();
		/*
		echo "</chart>"."\n";
		echo "</charts>"."\n";
		...
		*/
		...
		if ($this->chrt_array["appearance"]["sanim"] == "true") {
			$chart["animation"]=array("enabled"=>"true", "durection"=>"1000");
		}
	...
	function getDefaultValue($series, $row)
	{
		...
		if( count($this->detailTablesData) )
		{
			...
			$TableName=GoodFieldName($this->chrt_array['tables'][0])."_details";
			if( count($dashSet->getDashboardElementData( $TableName )) || !$this->dashTName ) {
	...
	function get_data($refr)
	{
		...
		$series=array();
		for ( $i=0; $i < count($this->arrDataSeries); $i++ ) {
			$series[]=array("name" => $this->arrDataLabels[$i], 
							"data" => $data[$i],
							"xScale"=>"0",
							"yScale"=>"1",
							"seriesType"=>"column"
			...
			if ($this->chrt_array["appearance"]["sval"]=="true")
				$series[count($series) - 1]["labels"] = array("enabled" =>  "true");
		}
		return $series;
	}
