...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("lh_users")
...
function SetLangVars($xt, $prefix, $pageName = "", $extraparams = "")
{
	...
	$xt->assign("Portuguese(Brazil)LANGLINK", $isBootstrap || "Portuguese(Brazil)" != $currentLang);
...
function xt_showchart($params)
{
	...
	if( !$params["singlePage"] )
	{
        ...
        anychart.onDocumentReady(function() {
			...
			xmlFile = Runner.getPageUrl('dchartdata') + '?chartname=".$params["chartname"]."&ctype=".$params["ctype"]."&rndVal=' + Math.random();
			$.get( xmlFile, function( data ) {
				//var v7conf = anychart.migrationTool.transformXml(data);
